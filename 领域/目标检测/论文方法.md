...
- **提高精度**   
  ...
  - **High Resolution Classiﬁer**        
     ...
     所有state-of-the-art的检测方法基本上都会使用ImageNet预训练过的模型（classifier）来提取特征，例如AlexNet输入图片会被resize到不足256 * 256，这导致分辨率不够高，给检测带来困难。   
  ...
  - **Convolutional With Anchor Boxes.**    
     YOLO(v1)使用全连接层数据进行bounding box预测（要把1470x1的全链接层reshape为7x7x30的最终特征），这会丢失较多的空间信息定位不准。YOLO v2借鉴了Faster R-CNN中的anchor思想,即RPN思想。   
     即舍弃全连接，用RPN代替    
     ![enter image description here](https://lh3.googleusercontent.com/9LR4T1ldGPq3FsxldC4qomEv25fHP2o-dPuN6xK3o0ge-OUhwrLbG4H3FgSkkTnQaz6XyLQXxmoe)     
     具体做法：   
     - 缩减网络，让图片输入分辨率为416 x 416，目的是让后面产生的卷积特征图宽高都为奇数，这样就可以产生一个center cell。因为作者观察到，大物体通常占据了图像的中间位置，可以只用一个中心的cell来预测这些物体的位置，否则就要用中间的4个cell来进行预测，这个技巧可稍稍提升效率。   
     
     - 使用卷积层降采样（factor 为32），使得输入卷积网络的416 x416图片最终得到13 x 13的卷积特征图（416/32=13）。     
     
     - 与RPN不同的是，不分别进行类别和框的计算，而是放在一起，由anchor box同时预测类别和坐标。因为YOLO是由每个cell来负责预测类别，每个cell对应的2个bounding box 负责预测坐标。YOLOv2中，不再让类别的预测与每个cell（空间位置）绑定一起，而是让全部放到anchor box中（回想YOLO中 最后输出7x7x30的特征，每个cell对应1*1*30，前10个主要是2个bounding box用来预测坐标，后20个表示该cell在假设包含物体的条件下属于20个类别的概率）。   
       ![enter image description here](https://lh3.googleusercontent.com/oXKGP6Vxv9srTsvd0robMJvd-Rri-KHNS8xi_Lyz5FUX4fMrWpgphhandRDa2uWsF7Px_lvzakMN)     
       加入了anchor boxes后，可以预料到的结果是召回率上升，准确率下降。我们来计算一下，假设每个cell预测9个建议框，那么总共会预测13 x 13 x 9 = 1521个boxes，而之前的网络仅仅预测7 x 7 x 2 = 98个boxes。  
  - **Dimension Clusters（维度聚类）**    
     ...
     随着k的增大，IOU也在增大（高召回率），但是复杂度也在增加。所以平衡复杂度和IOU之后，最终得到k值为5。  
  - **Multi-ScaleTraining**    
   原始YOLO网络使用固定的448 x 448的图片作为输入，加入anchor boxes后输入变成416 x 416，由于网络只用到了卷积层和池化层，就可以进行动态调整（检测任意大小图片）。为了让YOLOv2对不同尺寸图片的具有鲁棒性，在训练的时候也考虑了这一点。

	不同于固定网络输入图片尺寸的方法，每经过10批训练（10 batches）就会随机选择新的图片尺寸。网络使用的降采样参数为32，于是使用32的倍数{320,352，…，608}，最小的尺寸为320 x 320，最大的尺寸为608 x 608。 调整网络到相应维度然后继续进行训练。

	这种机制使得网络可以更好地预测不同尺寸的图片，同一个网络可以进行不同分辨率的检测任务，在小尺寸图片上YOLOv2运行更快，在速度和精度上达到了平衡。
  - **Fine-Grained Features（细粒度特征）**      
    修改后的网络最终在13 x 13的特征图上进行预测，虽然这足以胜任大尺度物体的检测，如果用上细粒度特征的话可能对小尺度的物体检测有帮助。Faser R-CNN和SSD都在不同层次的特征图上产生区域建议以获得多尺度的适应性。YOLOv2使用了一种不同的方法，简单添加一个 passthrough layer，把浅层特征图（分辨率为26 x 26）连接到深层特征图。

	passthroughlaye把高低分辨率的特征图做连结，叠加相邻特征到不同通道（而非空间位置）  ，类似于Resnet中的identity mappings。这个方法把26 x 26 x 512的特征图叠加成13 x 13 x 2048的特征图，与原生的深层特征图相连接。
- 提高速度   
...
YOLOv2使用了一个新的分类网络作为特征提取部分，参考了前人的工作经验。类似于VGG，网络使用了较多的3 x 3卷积核，在每一次池化操作后把通道数翻倍。借鉴了network in network的思想，网络使用了全局平均池化（global average pooling）做预测，把1 x 1的卷积核置于3 x 3的卷积核之间，用来压缩特征。使用batch normalization稳定模型训练，加速收敛，正则化模型。   
  ![enter image description here](https://lh3.googleusercontent.com/C3wLq2SDU6GobZSZyKvwi0YolRxvX9dO4PUjT4H9GjLYHMfIPdlKi2Xrmczg9Tx_VPAetnAjKr6X)    

- 更强大（分类改进）  
 

### Yolo v3   
主要改进有：1. 网络结构改进调整； 2：利用多尺度特征进行对象检测； 3.对象分类用Logistic取代了softmax。   
- 新网络   
  新网络结构借鉴了残差网络结构，加入了跳跃连接。    
  ![enter image description here](https://lh3.googleusercontent.com/4wCWTs9ovgVvLOhVK-Aw-_cfKdEBchevNPd3he7m4NshzuV05hkVmHRnxfUYwSVdc7i4Np8XG2af)    
- 利用多尺度特征进行对象检测     
   ![enter image description here](https://lh3.googleusercontent.com/GgNll1gRY6kcQSowZREngxTZXrmHv1WZ9bevzYRCipQgQpD_2d7-ZTErXclleYkKZCJMp3rzw6yS)
...
- 输入输出映射   
  ![enter image description here](https://lh3.googleusercontent.com/9aDzP4F3EPi4tr9GadSIslRvJhJ2YSuTAcljQdG9UAVUrbhDb5Lq0B0p0UrfFYz9X0ioirvddf8B)
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTg5MDYwOTMyMCwtMTU2NTg4NDg2OCwxOD
U4NTA3MjA4LDE2OTI4NzMzMTQsLTE4NjMxMzEwNjgsLTk2MjQ2
NTgyOCwxNDI0OTcwMjg5LDE2OTA5NTM3NDgsMTAxNTMxNTI2NS
wxNDgwNzM2MjM5LC0zNTI3MTYyMDIsLTU3NTQ3NTQ1OCwtOTI4
MjAwNzE0LDIxMTA0MDA3OTksLTEyMzg3OTQzMzAsODk5ODg2MT
YsLTE1ODQyODY0MTEsODA0NDUzMjgwLC03MDc2Nzk1MDEsMTQz
MDc4MzA4NF19
-->