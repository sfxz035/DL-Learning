...
- **提高精度**   
  ...
  - **Convolutional With Anchor Boxes.**    
     ...
     - 与RPN不同的是，不分别进行类别和框的计算，而是放在一起，由anchor box同时预测类别和坐标。因为YOLO是由每个cell来负责预测类别，每个cell对应的2个bounding box 负责预测坐标。YOLOv2中，不再让类别的预测与每个cell（空间位置）绑定一起，而是让全部放到anchor box中（回想YOLO中 最后输出7*7*30的特征，每个cell对应1*1*30，前10个主要是2个bounding box用来预测坐标，后20个表示该cell在假设包含物体的条件下属于20个类别的概率）。   
       ...
       加入了anchor boxes后，可以预料到的结果是召回率上升，准确率下降。我们来计算一下，假设每个cell预测9个建议框，那么总共会预测13 * 13 * 9 = 1521个boxes，而之前的网络仅仅预测7 * 7 * 2 = 98个boxes。
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjYxODgzODYyLDE2OTA5NTM3NDgsMTAxNT
