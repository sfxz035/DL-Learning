...
### SRGAN   
超分辨中过于平滑的问题，MSE使重建结果有较高的信噪比，但是缺少了高频信息，出现过度平滑的纹理？？超分辨BN层的问题？？   该网络允许修复更高频信息
- **输入输出尺寸**  
  - 输入  
    输入是未经预处理到与HR图像同等尺寸大小的LR图像 
  - 输出经过上采样层得到  
 ...
 - **训练细节**  
   - **训练数据**  
    ...
    - 
 - **损失**    
   该网络损失定义了一种感知损失loss 分为 内容loss 和 对抗网络loss，内容loss又分为mse loss和vgg loss
	- 感知损失   
...
- 网络相关
	- 网络参数  
	  输入输出为RGB的三通道图像块，48x48x3，输出(48xscale)x(48xscale)x3。  在最终单尺度模型EDSR中，通过设置B=32个残差块，F=256个通道。
	  对输入图像,目标图像进行预处理，所有图片减去一个均值。( We pre-process all the images by subtracting the mean RGB value of the DIV2K dataset）
	- 损失  
	    使用L1损失  
	- 网络结构  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTY0Njg0MDM0MiwtMTI0OTU1NDk3Miw0Nz
U1NzMzNjgsMTQwNTA0NzI4MSw2NTMyMjI3MCw2ODg1NTIwMiw1
MTU3OTEyMTksODMzNTAzMDI3LC0xMzY3MjYzNzc2LDExODg5MD
U3MjAsNzY0MTc2NDg2LDc2MTkwMjgyMCwtMjQwNzIzMDc4LDU3
ODg4NTgxOSwxNjEyNDkzODMyLC0xMTgzMTk5MjU5LC0zMTU0MD
g2NjYsMjE0MzQ1OTg1Myw2MzI1MDEyNjIsMTg1NjYwNjcxMV19
