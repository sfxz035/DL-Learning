...
### 损失  
  - 第一种  
	   - 判别器损失：  $E_{x \sim P_{data}}log(D(x))+E_{x \sim P_{G}}log(1-D(G(z))$  
	   - 生成器损失： $E_{x \sim P_{G}}log(D(G(z)))$
 - 第二种   
...
- 原始wgan网络     
   ...
   - 损失  
     ...
     - 理论  
         在理论分析中D()是一个Lipschitz连续（导数存在上界）的函数，这个函数作者是通过神经网络，通过改变权重w，学习这个函数的近似。
...
### 主要内容  
  将原始GAN中的概率全改成了条件概率，  
  $\displaystyle\max _{D} \left \{  E_{x \sim P_{data}}logD(x|y)+E_{x \sim P_{G}}log(1-D(x|G(z)))\right \}$  ？？？
...
- 相关特点  
 ...
 - 问题  
   文章说此网络为条件GAN，条件体现在哪里？体现label上？## cycleGAN  
   该网络针对非成对数据。   
   直接使用不成对的数据是不奏效的。网络会直接**忽略输入，随机产生输出**！所以，我们还得对网络增加**限制（constraint）**才行。所以该文章思想

<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE1MDkxMjkzNDQsLTU1MjE3NTM4Nyw2ND
MwMTEwODEsMjEzMDQ5Njg0NCwtMjE0NDc0NDAwMCw4OTczNDQ3
NzcsLTI2OTg2NDAyMiw2ODU3NDg3ODksMTA5NjgzNjM0MCw3MD
Q1MTA3MTIsNDI2NTc4MDQwLC0yMTE0NDExNDIsLTEwNTkyODEy
ODksLTYwNTgwMDgxMCw2MTkzODc5MjMsLTk5MDY1ODgyNiw2NT
c0MzUxNTksMTYwOTUxMTg5NSw2MDYyMTg1ODMsNDI4MjkxOTAy
XX0=
-->