...
###  **方法**      
该方法使用的是**配对数据**。
  判别器判别时，除了输入生成器输出外，加入条件输入(这篇文章是判别器输入加入生成器输入)，对判别器进行一个约束，从而达到对生成器的约束。使得生成器生成的结果与输入有一定的对应性。提高了用户控制能力。  
   其中z为输入，x为条件输入。
...
网络结构如下图  
 ...
 此处缺图！！！！！！！！！！https://photos.app.goo.gl/3nhAZctLr6vwmxYE8
 - **生成器结构**  
   使用Unet作为生成器  
   - 使用Unet原因  
     输入和输出图像的外表面(surface appearance)应该不同而潜在的结构(underlying structure)应该相似，对于image translation的任务来说，输入和输出应该共享一些底层的信息，因此使用Unet这种跳层连接(skip connection)的方法
 - **判别器结构**  
...
### **损失**  
   ...
   - object function为：  
    ...
    - 小结  
      GAN其实是一种相对于L1 loss更好的判别准则或者loss。有时候单独使用GAN loss效果好，有时候与L1 loss配合起来效果好。在pix2pix中，作者就是把L1 loss 和GAN loss相结合使用，因为作者认为L1 loss 可以恢复图像的低频部分，由于L1强制低频的正确性，从而促使GAN loss只关注高频部分，更准确的恢复图像的高频部分。为了对高频部分建模，将注意力限制在局部图像块上，也就是该文的patch结构。。
## cycleGAN  
   该网络针对非成对数据。   
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE5MTY0MjUyNzUsMTAyMzgxNDE4MywxMj
E1NDQ0Njc3LDQ3ODUzMDgzMSwtMTU4NDgzMjI5MSwtMjAwNzgy
OTMxNCwxODU5NTc3NjA3LC0xNDM3NDY1OTI2LC0xOTQ2NDkyMz
E3LDExMTc4MDE4ODEsLTE4MTQyODExMzQsNjk5Mjc3NzcxLDIw
NjA4ODk4MjIsMTIwNDg2NDE5MywtNTU1Njc2OTc0LDY1OTQ3NT
IxNywxMTAxNjA4NjMxLDY4NDgyODQzMiwtMTI1Nzk4MjIwOCwt
NDI0Njk0NzIyXX0=
-->