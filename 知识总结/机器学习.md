	...
	- 损失函数  
      ...
      - 负对数似然函数：  
         $$L(\omega)    =  -log(\prod_{i=1}^N{[\sigma(x_i)]^{y_i}[1-\sigma(x_i)]^{1-y_i}}) \\ =-\sum_{i=1}^{N}[y_i log(\sigma(x_i))+(1-y_i)log(1-\sigma(x_i))] \\=-\sum_{i=1}^{N}{[y_ilog\frac{\sigma(x_i)}{1-\sigma(x_i)}+log(1-\sigma(x_i))]}$$  
          $$L(\omega)   = -log(\prod_{i=1}^N{[\sigma(x_i)]^{y_i}[1-\sigma(x_i)]^{1-y_i}}) \\ =-\sum_{i=1}^{N}[y_i log(\sigma(x_i))+(1-y_i)log(1-\sigma(x_i))] $$  
         代入$\sigma(x_i)$，得到  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTM1MzU2NDg5OSwxMzk1MjE3MzkwLDk0Nz
