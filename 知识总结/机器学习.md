...
- **Boosting 策略要解决的两个基本问题**：  
	 ...
	 **AdaBoost如何解决以上问题：**
	3. 每一轮如何改变数据的权值或概率分布？——开始时，每个样本的权值是一样的，AdaBoost 的做法是提高上一轮弱分类器错误分类样本的权值，同时降低那些被正确分类样本的权值。
	4. 如何将弱分类器组合成一个强分类器？—— AdaBoost 采取加权表决的方法（加法模型）。具体的，AdaBoost 会加大分类误差率小的基学习器的权值，使其在表决中起到更大的作用，同时减小分类误差率大的基学习器的权值。  
	
	**AdaBoost算法思路**  
	- 输入：训练集`$T={(x1,y1),..,(xN,yN)}, xi ∈ R^n, yi ∈ {-1,+1}$`
	1. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTYzNDY0NTUwMiwyMDQwNzI0MzE2LDE0Nz
I1MzAxMSwyOTEzMjMxMjYsLTc2OTIzNzI0NywxMTAwOTQzMzM4
LC0xNjI5MDE2MTc4LC0xMDgwNzI5ODg0LDQxNzU0MzkwMywtMT
UyMTA1MzE2OSw1OTcwMDEzMjUsMTY2NjczMzE0NywtNDAxNDE4
ODM3LC0xMTQyMTI2MTIwLDQ2MDcwMDM4LC0xOTEzMzIyMzMzLD
MwMjI2ODQyMSwtNzI5MjA1OTc5LDE1OTg2NzIxMSwtMjIyMDky
MzA2XX0=
-->