...
实际上分类误差率就等于所有**分类错误的数据的权值之和**
		...
		- iv) 更新训练集样本的权值分布（作为下一个分类器的训练集）：  
		  ...
		  >其中 `Z_m` 为**规范化因子**，使 `D_m+1` 成为一个**概率分布**，类似 `Softmax` 函数
			...
			另外：${\color{Blue}y_iG_m(x_i) }=\left{\begin{matrix} 1,& G_m(x_i)=y_i \ -1,& G_m(x_i)\neq y_i \end{matrix}\right$
		  
### **Bagging，Boosting二者之间的区别**

<!--stackedit_data:
eyJoaXN0b3J5IjpbMTY0OTc2NDMwOSwxOTkzODIxOTU0LDIxMT
g5NjQyNTIsMjA0MDcyNDMxNiwxNDcyNTMwMTEsMjkxMzIzMTI2
LC03NjkyMzcyNDcsMTEwMDk0MzMzOCwtMTYyOTAxNjE3OCwtMT
A4MDcyOTg4NCw0MTc1NDM5MDMsLTE1MjEwNTMxNjksNTk3MDAx
MzI1LDE2NjY3MzMxNDcsLTQwMTQxODgzNywtMTE0MjEyNjEyMC
w0NjA3MDAzOCwtMTkxMzMyMjMzMywzMDIyNjg0MjEsLTcyOTIw
NTk3OV19
-->