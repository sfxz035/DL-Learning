	...
	- 损失函数  
      ...
      - 负对数似然函数：  
         $$L(\omega)    =  -log(\prod_{i=1}^N{[\sigma(x_i)]^{y_i}[1-\sigma(x_i)]^{1-y_i}}) \\ =-\sum_{i=1}^{N}[y_i log(\sigma(x_i))+(1-y_i)log(1-\sigma(x_i))] \\=-\sum_{i=1}^{N}{[y_ilog\frac{\sigma(x_i)}{1-\sigma(x_i)}+log(1-\sigma(x_i))]}$$  
          $$L(\omega)   = -log(\prod_{i=1}^N{[\sigma(x_i)]^{y_i}[1-\sigma(x_i)]^{1-y_i}})$$  
         代入$\sigma(x_i)$，得到  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE4NTMzMzg4NzQsMTM5NTIxNzM5MCw5ND
c4NTM1MzIsNDM4ODQ0OTI4LC0zNzEzNDkzMjIsMzE3NDI2Njc4
LC0yMDMwOTc4NjcsMTMyNzQyMTgyNSwtMTA2MDkwNTU1MywtMT
U4MjQwNTA3MiwxOTU0MzE4NTgzLC0xOTAyMzc4OTc5LC05MDAw
NzMzMSwxMTcyNTQzNzg5LDIwNzYyMjMzNjUsMTc3NDU5OTAyNS
wtNjcxMjc0NDQsMjI5NjgxNjU0LDExNDgyMzg5NDAsMTQwNTQ3
OTgzMl19
-->