...
- 2 求导进而求 `L` 对 `(w,b)` 的极小  
  ...
  $\begin{aligned} L(w,b,{\color{Red} \alpha}) &=\frac{1}{2}w^Tw-\sum_{i=1}^N{\color{Red} \alpha_i}[y_i(w^Tx_i+b)-1]\ &=\frac{1}{2}w^Tw-w^T\sum_{i=1}^N \alpha_iy_ix_i-b\sum_{i=1}^N \alpha_iy_i+\sum_{i=1}^N \alpha_i\ &=\frac{1}{2}w^Tw-w^Tw+\sum_{i=1}^N \alpha_i\ &=-\frac{1}{2}w^Tw+\sum_{i=1}^N \alpha_i\ &=-\frac{1}{2}\sum_{i=1}^N\sum_{j=1}^N \alpha_i\alpha_j\cdot y_iy_j\cdot {\color{Red} x_i^Tx_j}+\sum_{i=1}^N \alpha_i \end{aligned}$  
- 3 求 `L` 对 `α` 的极大，即  
    $\begin{aligned} &\underset{\alpha}{\max} \quad -\frac{1}{2}\sum_{i=1}^N\sum_{j=1}^N \alpha_i\alpha_j\cdot y_iy_j\cdot x_i^Tx_j+\sum_{i=1}^N \alpha_i\ & \mathrm{s.t.}\quad; \sum_{i=1}^N \alpha_i y_i=0,  {\color{Red} \alpha_i \geq 0},\quad i=1,2,\cdots,N \end{aligned}$  
    - 该问题的对偶问题  
      $\begin{aligned} &{\color{Red} \underset{\alpha}{\min} } \quad \frac{1}{2}\sum_{i=1}^N\sum_{j=1}^N \alpha_i\alpha_j\cdot y_iy_j\cdot x_i^Tx_j-\sum_{i=1}^N \alpha_i\ & \mathrm{s.t.}\quad; \sum_{i=1}^N \alpha_i y_i=0,  {\color{Red} \alpha_i \geq 0},\quad i=1,2,\cdots,N \end{aligned}$  
      于是，标准问题最后等价于求解该**对偶问题**  
 - 4 求解：设 `α` 的解为 `α*`，则存在下标`j`使`α_j > 0`，可得标准问题的解为：  
   $\begin{aligned} w^*&=\sum_{i=1}^N \alpha_i^*y_ix_i\ b^*&={\color{Red} y_j}-\sum_{i=1}^N \alpha_i^*y_i(x_i^T{\color{Red} x_j}) \end{aligned}$  
   可得分离超平面及分类决策函数为：
    
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTM0MjAwMTQ3NCwtMTE5ODg1NTE2NiwtOT
EzMzg4MDYsMTUwMTQyNDM4OCwtMjExMDM5MjcyMSwxNzUzODg1
Nzg3LDk3NTEzNzI4NiwxNzM0NDAxNzQ1LDEwMTUzNTU0NSwtOD
Q0NzYzNTksLTE0MTc5NTM5ODMsMTE3MjIzMTM1MywxMDEwODg1
MTEwLDEyNjY3OTMzOTcsLTE2Nzg2NzczNzYsLTE2MTA4NDM3MT
YsMTE0MzU4NTExNiwtMjAzNzg0MjM0NywxMTA2ODgyMzE2LDE4
MzE4MDM4OThdfQ==
-->