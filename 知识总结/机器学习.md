...
- **CART算法**
	...
	-  **CART树的构建**：  
		- **假设输入空间已经划分**： 
			...
			- 回归树——输出结果为连续型数据  
			  基本思想：选取是方差最小的特征作为分裂二叉树的分裂特征。方差越小，说明二分之后的子样本的“差异性”越小。  
			  方差计算方式：  
			  方差公式为		 ![enter image description here](https://github.com/sfxz035/DL-Learning/raw/master/picture/1556618509%281%29.jpg)  
			    μ表示样本集S中预测结果的均值，yk表示第k个样本预测结果。 
			    对于含有NN个样本的样本集SS，根据属性AA的第ii个属性值，将数据集SS划分成两部分，则划分成两部分之后方差计算：  
	...
	- 
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwNzg2MzM4NCwxNzYzNjQxMjAzLC02NT
cxOTY4NDgsLTE2MzQ1MzEyNDQsMTM5MDE4MTYxMCwtNzQ5MzI1
MTk2LC04OTQ5NjQxODUsMTkyNDg1MjYwOSw4ODkzNTY3NjEsMT
A2MDMxODEyOSwtODM0MDM5MTk5LDE1NjA1NjQ4MzgsLTc5OTM4
NzgxNCwtMzM4MTEzOTQwLC0xMTI2ODYzMjM2LDUzNjE2ODc4OS
w5ODEyOTc0MDcsLTI3OTI1NTM0MSwxMDcxNTIyMjkxLC00Nzkz
OTkyNjFdfQ==
-->