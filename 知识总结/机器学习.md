...
- **代表算法/模型**：1. 提升方法 AdaBoost； 2. 提升树；3. 梯度提升树 GBDT 
...
实际上分类误差率就等于所有**分类错误的数据的权值之和**
		...
		- iv) 更新训练集样本的权值分布（作为下一个分类器的训练集）：  
		  $\begin{aligned} D_{{\color{Red}m+1}}=(w_{m+1,1},\cdots,w_{m+1,i},\cdots,w_{m+1,N})\ 
;w_{{\color{Red}m+1},i}=\frac{w_{{\color{Red}m},i}\cdot\exp(-\alpha_{\color{Red}m}\cdot{\color{Blue}y_iG_m(x_i) })}{Z_{\color{Red}m}} \end{aligned}$  
		其中$\begin{aligned} D_{{\color{Red}m+1}}&=(w_{m+1,1},\cdots,w_{m+1,i},\cdots,w_{m+1,N})\ w_{{\color{Red}m+1},i}&=\frac{w_{{\color{Red}m},i}\cdot\exp(-\alpha_{\color{Red}m}\cdot{\color{Blue}y_iG_m(x_i) })}{Z_{\color{Red}m}} \end{aligned}$
		- 
		  
### **Bagging，Boosting二者之间的区别**

<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIxMTg0NTEwNiwyMTE4OTY0MjUyLDIwND
A3MjQzMTYsMTQ3MjUzMDExLDI5MTMyMzEyNiwtNzY5MjM3MjQ3
LDExMDA5NDMzMzgsLTE2MjkwMTYxNzgsLTEwODA3Mjk4ODQsND
E3NTQzOTAzLC0xNTIxMDUzMTY5LDU5NzAwMTMyNSwxNjY2NzMz
MTQ3LC00MDE0MTg4MzcsLTExNDIxMjYxMjAsNDYwNzAwMzgsLT
E5MTMzMjIzMzMsMzAyMjY4NDIxLC03MjkyMDU5NzksMTU5ODY3
MjExXX0=
-->