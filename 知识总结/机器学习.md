	...
	- 损失函数  
      ...
      - 负对数似然函数：  
         $$L(\omega)    =  -log(\prod_{i=1}^N{[\sigma(x_i)]^{y_i}[1-\sigma(x_i)]^{1-y_i}}) \\ =-\sum_{i=1}^{N}[y_i log(\sigma(x_i))+(1-y_i)log(1-\sigma(x_i))] \\=-\sum_{i=1}^{N}{[y_ilog\frac{\sigma(x_i)}{1-\sigma(x_i)}+log(1-\sigma(x_i))]}$$  
          $$L(\omega)   = -log(\prod_{i=1}^N{[\sigma(x_i)]^{y_i}[1-\sigma(x_i)]^{1-y_i}}) \\ =-\sum_{i=1}^{N}[y_i log(\sigma(x_i))+(1-y_i)log(1-\sigma(x_i))] =-\sum_{i=1} $$  
         代入$\sigma(x_i)$，得到  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTk0OTg3ODkyLDEzOTUyMTczOTAsOTQ3OD
UzNTMyLDQzODg0NDkyOCwtMzcxMzQ5MzIyLDMxNzQyNjY3OCwt
MjAzMDk3ODY3LDEzMjc0MjE4MjUsLTEwNjA5MDU1NTMsLTE1OD
I0MDUwNzIsMTk1NDMxODU4MywtMTkwMjM3ODk3OSwtOTAwMDcz
MzEsMTE3MjU0Mzc4OSwyMDc2MjIzMzY1LDE3NzQ1OTkwMjUsLT
Y3MTI3NDQ0LDIyOTY4MTY1NCwxMTQ4MjM4OTQwLDE0MDU0Nzk4
MzJdfQ==
-->