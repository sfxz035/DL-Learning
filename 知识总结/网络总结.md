...
- **主要思想**
	...
	NLnet的网络结构采用的是自注意力机制来建模像素对关系。non local这种操作，就是在提取某处特征时利用其周围点的信息，这个“周围”既可以是时间维度的，也可以是空间维度的。
...
通过严格的实验分析，作者发现non-local network的全局上下文在不同位置几乎是相同的，这表明学习到了无位置依赖的全局上下文，造成了大量的计算浪费。**虽然non-local block想要计算出每一个位置特定的全局上下文，但是经过训练之后，全局上下文是不受位置依赖的。**    
### GCNet    
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE0NTY4NTEzOTUsNjgxMTY1MzU4LC0xMz
MyNDMyOTQzLC0xNjc5NTkzOTMxLDIxMDQ4NTAzOTQsLTM0NjE1
ODgzNCwyMjUzOTI5MiwtMTIwODgwMTY3Niw4MjQwNjgyNzIsLT
EwMDM1NzQ1MjEsMTcwOTY3NzQzMyw0NzcyMzY0OCwtMTk4NjU3
NjU2NywtMTc5MTc2NDcyNCw5NTYxNjc1NDMsOTc2MTU2MzM1LD
E2NTQ1MDExNzQsLTU0NjYzOTM0LDUwNzUxMjkzNCwtNDY4NzM3
NDBdfQ==
-->