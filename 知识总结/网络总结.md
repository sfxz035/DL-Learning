...
- **使用场景**     
   ...
   >In practice, we have found that employing this factorization does not work well on early layers, but it gives very good results on medium grid-sizes (On ![[公式]](https://www.zhihu.com/equation?tex=m%5Ctimes+m) feature maps, where ![[公式]](https://www.zhihu.com/equation?tex=m) ranges between 12 and 20).
- **pool**   
...
- **主要思想**
	...
	non local这种操作，就是在提取某处特征时利用其周围点的信息，这个“周围”既可以是时间维度的，也可以是空间维度的。
...
- **主要内容**    
   ...
   - **本文方法**     
      ...
      - 具体实现   
        ...
        >这里y_i就是公式(1)中的输出结果。“+x_i”表示残差连接。这个残差连接使得我们可以将这个Non-local Block很方便的插入已有的预训练模型中，而不会破坏模型原有的操作。    
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTI3OTY4MTQwLDQ0MTAzMzk2NywtMTU2Mz
A2NTksLTUyMzYwNTgxNywtMjA3NjkwMDU2NiwtNjc1MzQ5ODI3
LC0xNTk5ODc2MjQ3LDEyMzg2NzQ4MDUsMTUxNTIyNjYzMCwtMj
cyMDA3MzI1LDEwMTg0MjQ1NjksLTE5MzE4OTgzNjUsLTk3Nzcw
NTAyNCwtMTg2ODQ2MDk3LDI3MjkxNDQ0MiwtMjA0MDE2OTg0Mi
wtNzUyODkwMCwxMzE4MjkxOTkwLDU2MTgzMjcyMyw2Nzc3MzIy
NDNdfQ==
-->