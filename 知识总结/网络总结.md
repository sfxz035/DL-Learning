...
- **pool**  
	- **要防止出现特征描述的瓶颈（representational bottleneck）。**所谓特征描述的瓶颈就是中间某层对特征在空间维度进行较大比例的压缩（比如使用pooling时），导致很多特征丢失。虽然Pooling是CNN结构中必须的功能，但我们可以通过一些优化方法来减少Pooling造成的损失。  
	  **为了避免representation瓶颈，在应用maximum或者average pooling之前需要将activation的维度进行增加**。  
	  ![](https://github.com/sfxz035/DL-Learning/raw/master/picture/20180521143822325.png)   
	  左图缺点：带来了一个representation瓶颈
\quadUnexpected text node: '&ThickSpace;'Unexpected text node: '&ThickSpace;'右图缺点：计算量增加三倍
...
**SGD,ADAM区别**
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTY4ODE1NjkyNiwxMDQ0NjUxMjM0LDIzNT
kyOTYyOCwtNDUyNTg0ODYyLC00MTI5MjU3NTcsLTEzOTk3MTE1
MzIsODQzMDk0NzY1LC0xNzIyMzc2ODIxLC0xMjc0MDQ3MTcyLD
ExMzk2MzU3ODMsLTE0NjQ2MTAyMTIsLTEyNDY5OTA0NSwtMTc4
NDUyMDI1NiwtMTcwOTMyODEzNCwtNDk0MTU5MTI1LC0xNTAyMz
k0NDQ0XX0=
-->