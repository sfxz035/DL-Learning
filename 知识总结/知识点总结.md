...
- **Xaiver****参数初始化方法**      
   ...
   **简介**：Xavier初始化的基本思想是保持输入和输出的方差一致，这样就避免了所有输出值都趋向于0，也不会导致梯度太小。Xavier初始化的推导过程是基于以下几个假设适用于激活函数sigmoid和tanh，不适用于ReLU。公式如下：
...
### **CNN****参数计算以及时间复杂度**     
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE1MTMzNjEwMDEsLTU3MjgzNTM5NywxOT
