 ...
 -  **高斯分布/截断正态分布**     
   ...
   **缺点：**方差的大小会影响网络性能。
	   - 如果初始的方差过小如0.1，参数分布会过小，就会导致在前向传播过程中，不同的层的输入不断减小，逐渐向0靠拢。   
		   导致：1. 由于梯度的计算会乘以输入（即上一层的输出），这会导致梯度过小，更新缓慢，甚至可能出现梯度弥散。 2. 由于靠近零分布，会使得激活函数sigmoid,tanh的梯度接近线性，失去非线性激活的  
	    - 那初始化的方差如果太大呢，就会使得每一层的输出越来越大。形如tanh激活函数，就会容易导致梯度饱和的现象
### **CNN****参数计算以及时间复杂度**     
<!--stackedit_data:
eyJoaXN0b3J5IjpbNDk2ODQwMzk1LDUyNjc3OTk2NSwtMTIzNz
c3OTI3NSwtNjcyNDcwNjg5LC0yMDczNjAwMTA2LDIwNTgyNjkx
NCw0OTM5MTgxOCw5MTIwNTE1MDIsLTE1NzI5NTI2MzQsLTE5Mz
AyMDg3MjIsLTEwNDM3ODE4NDIsMTczNzM5OTA2Nyw3ODI1MTAy
MDYsLTEzMzI0NzQwNDAsLTEyNTE1NTc1OTAsMTM3NTUzMTUyMS
w0Nzk0OTI3MTYsLTQyNjgxMzEwMSwxMDMzOTUxNTk4LDUxNzEx
NDk3OV19
-->